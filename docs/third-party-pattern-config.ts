 ...
 * When scraping providers return URLs that are proxied through external
 * services, this configuration allows the framework to extract the original
 * direct URLs.
 *
 *
 * WHY THIS EXISTS
 ...
 *   6. The captured URL is decoded
 *      (potentially multiple times for nested encoding)
 ...
 *
 * Example A:
 *   https://proxy.example.com/stream/https%3A%2F%2Freal.com%2Fvideo.m3u8
 *
 *                   Origin             Path         Encoded Real URL
 *
 * Example B:
 *   https://api.proxy.net/v1/proxy?url=https://real.com/video.m3u8&token=xyz
 *
 ...
 * Ask yourself:
 *
 *   "Will this pattern ONLY appear on this specific domain?"
 *
 ...
 *   - Example:
 *       Only "hlsproxy3.asiaflix.net" uses "/m3u8-proxy?url="
 *
 * Use WILDCARD ("*") if:
 *
 ...
 *
 * URL:
 *   https://proxy.com/stream/https%3A%2F%2Freal.com%2Fvideo.m3u8
 *
 * Pattern:
 *   /\/stream\/(.+)$/
 *
 *   - Matches literal "/stream/"
 *   - Captures the real URL
 ...
 *
 * URL:
 *   https://proxy.com/api/v1/proxy?url=https://real.com&token=abc
 *
 * Pattern:
 *   /\/proxy\?url=(.+?)(?:&|$)/
 *
 *   - Matches "/proxy?"
 *   - Matches "url="
 *   - Captures the real URL (non-greedy)
 ...
 *
 * URL:
 *   https://api.proxy.net/api/v2/proxy?url=https://real.com
 *
 * Pattern:
 *   /\/api\/[^/]+\/proxy\?url=(.+)$/
 *
 *   - Matches "/api/"
 *   - Matches any version (v1, v2, etc)
 *   - Captures the URL
 *
 *
 ...
 * ```ts
 * const testUrl =
 *   "https://your-proxy.com/your/structure?url=https%3A%2F%2Freal.com";
 *
 ...
 *
 *
 * 3. Using greedy capture with query params
 *
 * Wrong:
 *   /\?url=(.+)&/
 *
 ...
 *
 *
 * 5. Overly specific patterns in wildcard
 *
 * Wrong:
 *   '*': [/\/api\/v2\/proxy\?url=(.+)/]
 *
 ...
 *
 * Test your regex:
 *   https://regex101.com/ (use JavaScript flavor)
 *
